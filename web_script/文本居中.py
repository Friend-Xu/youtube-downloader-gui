...
"""
    ...
    for line in lines:
        # If the line is empty, add a simple line break
        if line.strip() == '':
            html_content += '<tr><td><br></td></tr>\n'
            continue
        ...
        for i, url in enumerate(urls):
            processed_text = processed_text.replace(f'URL_PLACEHOLDER_{i}',
                                                    f'<a href="{url}" target="_blank">{url}</a>')

        # Generate HTML table row with aligned links
        html_content += f'<tr class="line"><td>{processed_text}</td></tr>\n'
